  ...
  <div class="row">
     <div class="col s12 push-m3 m6">
      <div class="card white lighten-1">
        <div class="card-content blue-text"> <span class="card-title center">Set Tray/Shelf Location Form: 
         <?php echo $_SESSION['user_id']; ?></span><p class="center">Update Internal Note 1 and Alternative Call Number fields with tray barcode</p>
          <div class="row">
            <style>
#hideMe {
   ...
   // echo "<pre>Final URL After Redirects:\n" . htmlspecialchars($final_url) . "</pre>";
    ...
    if ($httpcode != 200) {
        ...
        if ($xml === false) {
         ...
         //  $item_url = (string) $xml->holding_data['link'];
           $item_url = htmlspecialchars($final_url);
           // $item_url = htmlspecialchars($final_url);
          //  echo '<p>item URL: '.$item_url.'</p>';
            if ($item_url) {
                // Construct scan-in URL
                $scan_in_url = $item_url . '&op=scan&library=' . urlencode($library) . '&circ_desk=' . urlencode($circ_desk);
               // echo "<p>__ Calling POST \"$scan_in_url\" ...</p>";
              
//////////// Begin GET/PUT Update Internal Note 1 and alternate call number
...
//$custom_internal_note_1 = $trayBarcode . ' - test';
...
if ($xml === false) {
    ...
    echo "<li class='list-group-item'><strong>Tray Barcode (ACN):</strong> " . htmlspecialchars($alternative_call_number) . "</li>";
...
//////////// End GET/PUT Update Internal Note 1 and alternate call number
...
else {echo '<p>Execption found : Return Item to Tammy</p><p>'.$put_response.'</p>';}
